...
//    private static final Logger LOG = LoggerFactory.getLogger(UserServiceImpl.class);
    ...
    public User getUser(String name) {
        for (User user : users) {
            if (user.getName().equals(name)) {
                System.out.println(String.format("服务端-返回处理结果：返回值=User name=%s, address=%s, phone=%s)", user.getName(), user.getAddress(), user.getPhone()));
                return user;
            }
        }
    ...
    public void refreshUsers() {
        System.out.println("服务端-不返回处理结果");
